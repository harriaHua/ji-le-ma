		...
		<view class="show" v-if="data.length>0">
			<view class="show-item" v-for="(item,index) in data">
				<view class="show-item-message" @click="sendaddr(index,flag)">
					...
					<text>{{item.toName}}&nbsp;&nbsp;{{item.phone.substr(0,3)}}&nbsp;{{item.phone.substr(3,4)}}&nbsp;{{item.phone.substr(-4)}}

						{{item.location}}{{item.detail}}</text>
					<!-- 数据显示 -->
		...
		methods: {
			...
			match() {
				...
				this.data=this.list.filter(item=>{
					return item.toName.includes(this.keyword)
					||item.phone.includes(this.keyword)
					||item.location.includes(this.keyword)
					||item.detail.includes(this.keyword)
				})
			}
		}
	}
